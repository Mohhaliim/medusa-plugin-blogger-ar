...
export const ArticleCard = (props) => {
    ...
    const title = props.article.title !== null? props.article.title : props.article.title_ar;
    const subtitle = props.article.subtitle !== null? props.article.subtitle : props.article.subtitle_ar;
            ...
            <a href={`/a/article-editor?id=${props.article.id.split("blog_article_")[1]}`} className="flex flex-col gap-2 h-full">
                ...
                <div className="flex flex-col gap-2 justify-between p-1 h-full">
                    <div className="flex flex-col gap-1">
                        <h3 className="text-lg leading-tight font-medium">{title?.length > 60 ?
                        title.slice(0, 60).trim() + "..." :
                        (title || <span className="text-gray-500/90">Missing title</span>)}</h3>

                        <p className="text-gray-600 leading-tight -mt-0.5">{subtitle?.length > 100 ?
                        subtitle.slice(0, 100).trim() + "..." :
                        (subtitle || <span className="text-gray-400">Missing subtitle</span>)}</p>
                    </div>
